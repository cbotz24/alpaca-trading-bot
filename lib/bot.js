...
async function getSnapshots(symbols,k,s){
  const unique=[...new Set(symbols)];const result={};
  for(let i=0;i<unique.length;i+=500){
    const chunk=unique.slice(i,i+500);
    const res=await fetch(DATA_BASE+'/v2/stocks/snapshots?symbols='+chunk.join(',')+'&feed=iex',{headers:headers(k,s)});
    const data=await res.json();
    if(data&&typeof data==='object')Object.assign(result,data);
  }
  return result;
}
async function getBars(sym,k,s){
  const end=new Date();const start=new Date(end.getTime()-4*60*60*1000);
  const url=DATA_BASE+'/v2/stocks/'+sym+'/bars?timeframe=1Min&start='+start.toISOString()+'&end='+end.toISOString()+'&limit=60&feed=iex';
  const data=await(await fetch(url,{headers:headers(k,s)})).json();
  return data.bars||[];
}
async function placeOrder(sym,qty,side,k,s){
  const res=await fetch(BASE+'/v2/orders',{method:'POST',headers:headers(k,s),body:JSON.stringify({symbol:sym,qty:qty.toString(),side,type:'market',time_in_force:'day'})});
  return res.json();
}
async function closePosition(sym,k,s){
  const res=await fetch(BASE+'/v2/positions/'+sym,{method:'DELETE',headers:headers(k,s)});
  if(res.status===204)return{success:true};
  return res.json();
}
async function getTodayBuys(k,s){
  const today=new Date().toISOString().split('T')[0];
  const orders=await(await fetch(BASE+'/v2/orders?status=filled&limit=100&after='+today+'T00:00:00Z&direction=desc',{headers:headers(k,s)})).json();
  const buys=new Set();
  if(Array.isArray(orders))orders.forEach(o=>{if(o.side==='buy')buys.add(o.symbol)});
  return buys;
}
...
function isNearClose(){
  const et=new Date(new Date().toLocaleString('en-US',{timeZone:'America/New_York'}));
  return et.getHours()*60+et.getMinutes()>=945;
}
function isBuyWindow(){
  const et=new Date(new Date().toLocaleString('en-US',{timeZone:'America/New_York'}));
  const d=et.getDay();if(d===5)return false;
  return et.getHours()*60+et.getMinutes()<840;
}
...
export async function runBotCycle(apiKey,secretKey,addLog){
  ...
  for(const[symbol,pos]of Object.entries(posMap)){
    ...
    // Skip positions with no available qty (pending sell order already in flight)
  ...
  const candidates=Object.entries(snaps)
    .filter(([sym,snap])=>{
      if(posMap[sym])return false;
      const price=snap?.latestTrade?.p||snap?.minuteBar?.c||0;
      const vol=snap?.dailyBar?.v||0;
      return price>=MIN_PRICE&&price<=maxPrice&&vol>=MIN_VOLUME;
    })
    .map(([sym,snap])=>({symbol:sym,price:snap?.latestTrade?.p||snap?.minuteBar?.c||0,dailyVol:snap?.dailyBar?.v||0}))
